...
class SaltAPI(object):
    ...
    def sync_code_hist(self, host, arg):
        ''' update code '''
        params = {'client': 'local_async', 'tgt': host, 'fun': 'cmd.run', 'arg': arg, 'expr_form': 'list'}
        obj = urllib.urlencode(params)
        content = self.postRequest(obj)
        jid = content['return'][0]['jid']
        hist_content = str(content)
        return jid,hist_content
    def sync_code(self, host, arg):
        ''' update code '''
        params = {'client': 'local_async', 'tgt': host, 'fun': 'cmd.run', 'arg': arg, 'expr_form': 'list'}
        obj = urllib.urlencode(params)
        content = self.postRequest(obj)
    ...
    def deliver_config_hist(self, host,arg):
        """deliver_config_hist
           local_async :异步任务
        ...
        hist_content = str(content)
        return   jid,hist_content
    def remote_execution_async(self, tgt, arg):
        ''' Shell command execution with async'''
